...
from base.notifications.helpers import (
    uzb_now, format_datetime, format_money,
    format_duration_minutes, format_prep_time,
)
...
class ShiftNotification:
    ...
    def _send_shift_start(cls, user_name):
        if not NotificationConfig.is_enabled('shift.start'):
            return

        date_str, time_str = format_datetime()
        text = messages.SHIFT_START.format(
            cashier_name=user_name,
            date=date_str,
            time=time_str,
        ).strip()

        cls._send_async(text, 'shift.start')

    @classmethod
    def _send_shift_end(cls, session):
        if not NotificationConfig.is_enabled('shift.end'):
            return
        ...
        text = messages.SHIFT_END.format(
        ...
        cls._send_async(text, 'shift.end')

    @classmethod
    def _send_shift_switch(cls, old_session, new_user_id, new_user_name):
        cls._send_shift_end(old_session)

        if not NotificationConfig.is_enabled('shift.switch'):
            return

        date_str, time_str = format_datetime()
        text = messages.SHIFT_SWITCH.format(
        ...
        cls._send_async(text, 'shift.switch')
        ...
        def _send():
            try:
                ...
                if not ok:
                    ...
                    logger.warning(f'Shift notification queued ({notification_type}): {error}')
            except Exception as e:
                NotificationQueue.add(text, notification_type)
                logger.error(f'Shift notification error ({notification_type}): {e}')
